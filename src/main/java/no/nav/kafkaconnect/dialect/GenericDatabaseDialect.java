...
package no.nav.kafkaconnect.dialect;

import no.nav.kafkaconnect.dialect.DatabaseDialectProvider.FixedScoreProvider;
import no.nav.kafkaconnect.sink.*;
import no.nav.kafkaconnect.sink.JdbcSinkConfig.InsertMode;
import no.nav.kafkaconnect.sink.JdbcSinkConfig.PrimaryKeyMode;
import no.nav.kafkaconnect.sink.metadata.FieldsMetadata;
import no.nav.kafkaconnect.sink.metadata.SchemaPair;
import no.nav.kafkaconnect.sink.metadata.SinkRecordField;
import no.nav.kafkaconnect.util.*;
import no.nav.kafkaconnect.util.ColumnDefinition.Mutability;
import no.nav.kafkaconnect.util.ColumnDefinition.Nullability;
import no.nav.kafkaconnect.util.ExpressionBuilder.Transform;
...
public class GenericDatabaseDialect implements DatabaseDialect {
  ...
  protected GenericDatabaseDialect(
    ...
    this.jdbcUrlInfo = DatabaseDialects.extractJdbcUrlInfo(jdbcUrl);
      ...
      tableTypes = new HashSet<>(Arrays.asList(JdbcSinkConfig.TABLE_TYPE_DEFAULT));
